...
function loremIpsumSentence2(numWords){
    var result = "";
    result += capitalize(getLoremWord()) + " ";
	var part1 = 0;
	if(chance(50)){
		// insert a comma or other punctuation within the sentence
		part1 = Math.floor(Math.random() * numWords-2);
		result += loremIpsum(part1);
		result += ", ";
	} else {
	    result += " ";
	}
	result += loremIpsum(numWords - part1 - 1);
    result += getLoremEnding();
    result += " ";
    return result;
}
...
function loremIpsumParagraph(numWords){
	...
	if(words > 10){
	    ...
	    result += loremIpsumSentence2(w);
	    words = words - w;
	} else {
	    result += loremIpsumSentence2(words);
	    words = 0;
	}
    }
    return result;
